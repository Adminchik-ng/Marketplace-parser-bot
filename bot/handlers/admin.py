...
async def process_ban_command(
    ...
    if arg_user.isdigit():
        banned_status = await get_user_banned_status_by_id(conn, telegram_id=int(arg_user))
    ...
    if banned_status is None:
        ...
        if arg_user.isdigit():
            await change_user_banned_status_by_id(conn, telegram_id=int(arg_user), banned=True)
        else:
...
async def process_unban_command(
    ...
    if banned_status is None:
        ...
        if arg_user.isdigit():
            await change_user_banned_status_by_id(conn, telegram_id=int(arg_user), banned=False)
        else:
