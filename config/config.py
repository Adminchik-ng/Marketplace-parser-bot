...
def load_config(path: str | None = None) -> Config | TestConfig:
    env = Env()
    
    # Определяем тип окружения по имени файла
    is_test = path and "test" in path
    ...
    if is_test:
        # Для тестового окружения используем только необходимые переменные
        db = DatabaseSettings(
